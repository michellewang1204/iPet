...
public class vaccine extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vaccine = (ArrayList<String>) bundle.get("vaccine");
        ...
        for (String s : vaccine) {
            System.out.println(s);
        }

        thedate = new ArrayList<String[]>();

        for (int n = 0; n < (vaccine.size() / 2); n++) {
            String[] temp = {vaccine.get(n * 2), vaccine.get(n * 2 + 1)};
            thedate.add(temp);
        }
            ...
            runOnUiThread(() -> {
                ...
                back.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        Intent i = new Intent(vaccine.this, health.class);
                        i.putExtras(bundle1);
                        startActivity(i);
                    }
                });
...
//                Toast.makeText(view.getContext(),parent.getSelectedItem().toString()/*這行可直接取得選中內容*/,Toast.LENGTH_SHORT).show();
    ...
    private void setAlarm() {
        int num = 50;
        for (String[] d : thedate) {
            Intent intent = new Intent(this, vaccinebroadcast.class);
        ...
        btSetTarget.setOnClickListener(v -> {
            new Thread(() -> {
                ...
                for (Calendar calendar : calendarView.getSelectedDates()) {
                    ...
                    for (int n = 0; n < vaccine.size() - 1; n++) {
                        if (vaccine.get(n).equals(finalInActiveDate) && vaccine.get(n + 1).equals(cat)) {
                            check = false;
                        }
                    }
                    if (check) {
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                con.run();
                                con.setDate(finalInActiveDate, pid, cat, "vaccine");
                            }
                        }
                        ).start();
                        vaccine.add(finalInActiveDate);
                        vaccine.add(cat);
                        bundle1.putStringArrayList("vaccine", vaccine);
                ...
                for (Calendar calendar : calendarView.getSelectedDates()) {
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            TimeSql con = new TimeSql();
                            con.run();
                            con.deleteDate(finalInActiveDate, pid, "vaccine");
                        }
                    }
                    ).start();
                    int index = vaccine.indexOf(finalInActiveDate);
                    vaccine.remove(finalInActiveDate);
                    vaccine.remove(index);
                    bundle1.putStringArrayList("vaccine", vaccine);
                    /**利用for迴圈找出指定元素之index*/
        ...
        btGetDay.setOnClickListener(v -> {
            ...
            for (Calendar calendar : calendarView.getSelectedDates()) {
                ...
                for (int n = 0; n < vaccine.size(); n++) {
                    if (vaccine.get(n).equals(finalInActiveDate)) {
                        category = category + " " + vaccine.get(n + 1);
                    }
                }
                if (category != null) {
